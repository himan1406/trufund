...
exports.getProfile = async (req, res) => {
  ...
  try {
    ...
    const userResult = await pool.query(
      `SELECT user_id, username, full_name, email, bio, profile_image, created_at
    ...
    const followersResult = await pool.query(
      `SELECT COUNT(DISTINCT f.user_id) AS followers_count
       FROM followers f
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    const updated = await pool.query(
      "SELECT user_id, username, full_name, email, bio, profile_image FROM users WHERE user_id = $1",
      [user_id]
    )
