        ...
        debounceRef.current = setTimeout(async () => {
            ...
            try {
                ...
                const [usersRes, hashtagsRes] = await Promise.all([
                    fetch(`${BASE}/api/users/search?q=${encodeURIComponent(query)}`,          { credentials: "include" }),
                    fetch(`${BASE}/api/posts/search/hashtags?q=${encodeURIComponent(query)}`, { credentials: "include" }),
                ])
